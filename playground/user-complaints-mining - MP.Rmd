...
Building a corpus, which is a collection of text documents VectorSource specifies that the source is character vectors.After that, the corpus needs a couple of transformations, including changing letters to lower case, removing punctuations/numbers 
...
if(!file.exists("./user-complaints-mining/myCorpus.Rds")) {
  ...
  #[MP] added another set of X's below but didn't run it
  myStopwords <- c(stopwords(language="en", source="smart"), "xxxx", "xxxxxxxxxxxx", "xxxxxxxx", "xxxxxxxxxxxxxxxx", "aaa") 
...
plot(fit)
rect.hclust(fit, k = 7) # Cut into 7 clusters -> 7 product types we're looking to map to
...
#Using Correlated Topics Models
ctm<- CTM(myDtm, k = 10)
term <- terms(ctm, 10) # first 10 terms of every topic

# first topic identified for every document (tweet)
topic <- topics(ctm, 1)
library(data.table)
topics <- data.frame(df$Product, topic)
