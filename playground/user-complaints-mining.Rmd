...
if(!file.exists("./user-complaints-mining/myCorpus.Rds")) {
  ...
  myStopwords <- c(stopwords(language="en", source="smart"), "xxxx", "xxxxxxxxxxxx", "xxxxxxxx")
...
if(!file.exists("./user-complaints-mining/lda10.Rds")) {
  df.new = df[ui,]
  lda <- LDA(dtm.new, k = 7) # identify topics
  saveRDS(lda, file = "./user-complaints-mining/lda10.Rds")
  gc()
} else {
  lda <- readRDS("./user-complaints-mining/lda10.Rds")
}  
...
```{r message=FALSE, warning=FALSE, linewidth = 90}
topic <- topics(lda, 5, threshold=.15)
